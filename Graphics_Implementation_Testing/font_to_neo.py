...
def collapse_planes_helper(n_planes, pal, colors, img_data):
    if len(img_data) == 0:
        ...
        colors.append(
            neo_pal.color_index(planes_to_color(img_data[0:n_planes])))
        return collapse_planes_helper(n_planes, pal, colors,
                                      img_data[n_planes:])
...
parser = argparse.ArgumentParser(
    description='Convert piskel png to Arduino code')
parser.add_argument(
    'png_filename', type=str, nargs=1, help='The name of the file to convert')
...
for gset in range(0, n_glyphsets):
    rotated_colors.append(
        list(zip(*colors[gset * ROWS_PER_GLYPH:(gset + 1) * ROWS_PER_GLYPH])))
...
print("GlyphColumn32 column_data[{}][{}] = {{".format(n_glyphsets,
                                                      len(rotated_colors[0])))
for gset in range(0, n_glyphsets):
    print("  {")
